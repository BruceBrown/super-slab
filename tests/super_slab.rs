...
#[test]
fn insert_get_many() {
    let mut slab = SuperSlab::with_capacity(5);
...
fn insert_with_vacant_entry() {
    let mut slab = SuperSlab::with_capacity(1);
    let key;
...
fn reserve_does_not_allocate_if_available() {
    // let mut slab = SuperSlab::with_capacity(5,2);
    // let mut keys = vec![];
    //
    // for i in 0..6 {
    ...
    // slab.reserve(8);
    // assert_eq!(10, slab.capacity());
    println!("reserve is not implemented for SuperSlab");
}
...
fn reserve_exact_does_not_allocate_if_available() {
    ...
    // slab.reserve(8);
    // assert_eq!(10, slab.capacity());
    println!("reserve_exact is not implemented for SuperSlab");
}
...
fn retain() {
    ...
    // when a small vec grows from 1 to 2, it actually goes from 1 to 4.
